...
#Acrobot
alphaValue = 0.02 #parameter gradient
sigma = 0.08 #parameter noise -update Fi
num_workers=300
decay_start=int(num_episodes/4)
################
...
def worker_toeplitz_GS(input_worker):
    """Explanations"""
    #Global variables:
    global numInput,numOutput,numHidden1,numHidden2
    global dim_hidden2_output, dim_input_hidden1, dim_hidden1_hidden2
...
def worker_GS(input_worker):
    ...
    '''
    #GS
    epsilon_wo= epsilon[0:dim_hidden2_output+numOutput].reshape((numHidden2+1,numOutput)) #the +1 is due to the bias  
    epsilon_wh= epsilon[dim_hidden2_output+numOutput:dim_hidden2_output+numOutput+dim_hidden1_hidden2+numHidden1+numHidden2+1].reshape((numHidden1+1,numHidden2+1))#the +1 is due to the bias 
    epsilon_wi= epsilon[dim_hidden2_output+numOutput+dim_hidden1_hidden2+numHidden1+numHidden2+1:].reshape((numInput+1,numHidden1+1))
...
def main_GS(seeds,epsilons,params):
    num_workers=len(seeds)
    ...
    input_worker = list(zip(epsilons,seeds_env,[params]*len(seeds)))
...
if __name__ == "__main__":
    ...
    #print('The number of workers is',len(reward_workers))        
    fitness = fitness_shaping_paper(range(num_workers))
        
    for episode in range (num_episodes):
        print('episode : ',episode)
    
        seeds = np.random.randint(1,size=num_workers)
        ...
        epsilons=GS_epsilons_ini[0:num_workers]      
        
        

        reward_workers,epsilon_wi,epsilon_wh,epsilon_wo =  [list(x) for x in  zip(*main_GS(seeds,epsilons,params))]
        ...
        print(expo_alpha(episode,alphaValue,alphaValue*0.1,num_episodes)*(1/(num_workers*sigma))*sum([eps*F*w for eps,F,w in zip(epsilon_wi,reward_workers,fitness)]))

        
        params[0] = params[0] + expo_alpha(episode,alphaValue,alphaValue*0.1,num_episodes)*(1/(num_workers*sigma))*sum([eps*F*w for eps,F,w in zip(epsilon_wi,reward_workers,fitness)])
        params[1] = params[1] + expo_alpha(episode,alphaValue,alphaValue*0.1,num_episodes)*(1/(num_workers*sigma))*sum([eps*F*w for eps,F,w in zip(epsilon_wh,reward_workers,fitness)])
        params[2] = params[2] + expo_alpha(episode,alphaValue,alphaValue*0.1,num_episodes)*(1/(num_workers*sigma))*sum([eps*F*w for eps,F,w in zip(epsilon_wo,reward_workers,fitness)])
