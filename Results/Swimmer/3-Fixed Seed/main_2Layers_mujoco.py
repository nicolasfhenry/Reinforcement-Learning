...
#Acrobot
alphaValue = 0.008 #parameter gradient
...
if __name__ == "__main__":
    ...
    for episode in range (num_episodes):
        ...
        #print(expo_alpha(episode,alphaValue,0.1*alphaValue,num_episodes)*(1/(num_workers*sigma))*sum([eps*F*w for eps,F,w in zip(epsilon_wi,reward_workers,fitness)]))        
        print('')
        ...
        params[0] = params[0] + expo_alpha(episode,alphaValue,0.1*alphaValue,num_episodes)*(1/(num_workers*sigma))*sum([eps*F*w for eps,F,w in zip(epsilon_wi,reward_workers,fitness)])
        params[1] = params[1] + expo_alpha(episode,alphaValue,0.1*alphaValue,num_episodes)*(1/(num_workers*sigma))*sum([eps*F*w for eps,F,w in zip(epsilon_wh,reward_workers,fitness)])
        params[2] = params[2] + expo_alpha(episode,alphaValue,0.1*alphaValue,num_episodes)*(1/(num_workers*sigma))*sum([eps*F*w for eps,F,w in zip(epsilon_wo,reward_workers,fitness)])
    ...
    save_obj(params,'params-v2Layers_{}_{}_{}_{}_{}_+_FS'.format(num_episodes,alphaValue,sigma,np.mean(reward_workers),np.median(reward_workers)))
    save_obj(reward_episode,'reward-v2Layers_{}_{}_{}_{}_{}_+_FS'.format(num_episodes,alphaValue,sigma,np.mean(reward_workers),np.median(reward_workers)))
