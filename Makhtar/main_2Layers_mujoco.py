...
#Acrobot
alphaValue = 0.001 #parameter gradient
sigma = 0.3 #parameter noise -update Fi
num_workers=100
################
...
if __name__ == "__main__":
    ...
    '''
    #params = [np.random.randn(numInput+numHidden1),np.random.randn(numHidden1+numHidden2),np.random.randn(numHidden2+numOutput)] # Adding +1 is to counter the bias in the correponding layer 
    ...
    for episode in range (num_episodes):
        ...
        print(params[0])
        print("update:")
        print(alpha(episode,alphaValue))
        print(alpha(episode,alphaValue)*(1/(num_workers*sigma))*sum([eps*F for eps,F in zip(epsilon_wi,reward_workers)]))
